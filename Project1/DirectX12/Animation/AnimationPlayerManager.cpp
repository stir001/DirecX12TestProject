...
{
	mThreadObject.push_back((std::thread([](std::list<AnimatiomInfo>& animInfo) {
...
{
	if (mThreadObject.front().joinable())
	{
		mThreadObject.front().join();
	}
}
...
void AnimationPlayerManager::WaitSafeFree()
{
	if (mIsThreadRun)
	{
		if (mThreadObject.back().joinable()) {
			mThreadObject.back().join();
			mIsThreadRun = false;
		}
		//mResult.wait();
		mIsThreadRun = false;
